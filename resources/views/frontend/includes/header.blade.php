                ...
                <ul class="flex items-center gap-6">
                    <li class="relative" id="floor-plans-dropdown">
                        ...
                        <div id="floor-plans-menu"
                            ...
                            <div class="floor-plan-items hidden">
                            </div>
                        </div>
                    </li>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (langToggleButton && langMenu) {
            ...
            document.addEventListener('click', function(event) {
                if (!langMenu.classList.contains('hidden') && !langToggleButton.contains(event
                    .target)) {
        ...
        if (floorPlansToggle && floorPlansMenu && floorPlansDropdown) {
            ...
            // Hide menu with delay on mouse leave to allow for submenu navigation
        ...
        function loadFloorPlans() {
            const loadingElement = floorPlansMenu.querySelector('.loading');
            const itemsContainer = floorPlansMenu.querySelector('.floor-plan-items');

            loadingElement.classList.remove('hidden');
            itemsContainer.classList.add('hidden');
            itemsContainer.innerHTML = '';

            fetch('{{ route('floor-plans.api') }}')
                ...
                .then(floorPlans => {
                    ...
                    if (floorPlans.length === 0) {
                        itemsContainer.innerHTML =
                            '<div class="px-4 py-2 text-gray-600">{{ __db('no_floor_plans_available') }}</div>';
                    ...
                    floorPlans.forEach(floorPlan => {
                        ...
                        const title = lang === 'ar' ? floorPlan.title_ar : floorPlan.title_en;

                        const floorPlanDiv = document.createElement('div');
                        floorPlanDiv.className = 'floor-plan-group';

                        const titleElement = document.createElement('div');
                        titleElement.className =
                            'floor-plan-title font-bold px-4 py-2 text-gray-800 hover:bg-gray-100 cursor-pointer flex justify-between items-center';
                        ...
                        filesContainer.className = 'floor-plan-files hidden bg-gray-50';

                        if (floorPlan.file_objects && floorPlan.file_objects.length > 0) {
                            floorPlan.file_objects.forEach(fileObj => {
                                const path = typeof fileObj === 'string' ? fileObj : (
                                    fileObj.path || fileObj['path']);
                                const title = typeof fileObj === 'string' ? path.split('/')
                                    .pop() : (fileObj.title || fileObj['title'] || path
                                        .split('/').pop());

                                const fileLink = document.createElement('a');
                                fileLink.href = `{{ asset('storage/') }}/${path}`;
                                fileLink.target = '_blank';
                                fileLink.className =
                                    'block px-8 py-2 text-sm text-gray-700 hover:bg-gray-200';
                                fileLink.textContent = title;
                                filesContainer.appendChild(fileLink);
                            });
                        } else {
                            const noFiles = document.createElement('div');
                            noFiles.className = 'px-8 py-2 text-sm text-gray-500';
                            noFiles.textContent = '{{ __db('no_files_available') }}';
                        ...
                        floorPlanDiv.appendChild(titleElement);
                        floorPlanDiv.appendChild(filesContainer);
                        itemsContainer.appendChild(floorPlanDiv);
                    });
                ...
                .catch(error => {
                    ...
                    loadingElement.classList.add('hidden');
                    itemsContainer.innerHTML =
                        '<div class="px-4 py-2 text-red-600">{{ __db('error_loading_floor_plans') }}</div>';
                    itemsContainer.classList.remove('hidden');
                });
        }
    });
