...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-2 mb-6">
        <div class="justify-between">
            <h2 class="font-semibold mb-0 !text-[22px]">{{ __db('countries') }}</h2>
        </div>
                        ...
                        @forelse($countries as $key => $country)
                            <tr class="odd:bg-[#F9F7ED] align-[middle]">
                                        ...
                                        <div class=" items-center">
                                            <label for="switch-{{ $key }}" class="relative inline-block w-11 h-6">
                                                <input type="checkbox" id="switch-{{ $key }}"
                                                    onchange="update_status(this)" value="{{ $country->id }}"
                                                    class="sr-only peer" {{ $country->status == 1 ? 'checked' : '' }} />

                                                <div
                                                    class="block bg-gray-300 peer-checked:bg-[#009448] w-11 h-6 rounded-full transition">
                                                </div>
                                                <div
                                                    class="dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition peer-checked:translate-x-5">
                                                </div>
                                            </label>
                                        </div>
                                    @endcan
                                ...
                                <td class="text-center">
                                    @can('edit_dropdown_options')
                                        <button data-modal-target="edit-country-{{ $country->id }}"
                                            data-modal-toggle="edit-country-{{ $country->id }}"
                                ...
                                <div id="edit-country-{{ $country->id }}" tabindex="-1" aria-hidden="true"
                                    ...
                                    <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
                                        ...
                                        <h2 class="text-xl font-semibold mb-4">{{ __db('edit_country') }}</h2>

                                        <form class="" method="POST"
                                            action="{{ route('dropdowns.options.update', $country) }}">
                                            @csrf
                                            @method('PUT')
                                            <div class="mb-4">
                                                <label
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <select name="status" class="w-full border border-gray-300 rounded p-2">
                                                    <option value="1" {{ $country->status ? 'selected' : '' }}>
                                                        {{ __db('active') }}</option>
                                                    <option value="0" {{ !$country->status ? 'selected' : '' }}>
                                            ...
                                            <div class="flex justify-start space-x-2 pt-4">
                                                ...
                                                <button type="button" data-modal-hide="edit-country-{{ $country->id }}"
                                                    class="btn text-md mb-[-10px] border !border-[#B68A35] !text-[#B68A35] rounded-lg h-12">{{ __db('cancel') }}</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
