...
<div class="flex flex-wrap items-center justify-between gap-2 mb-6">
    ...
    <div>
        @can('add_dropdown_options')
            <button data-modal-target="add-country" data-modal-toggle="add-country" class="btn me-8 text-md mb-[-10px] !bg-[#B68A35] text-white rounded-lg ">
                            ...
                            <div id="edit-country-{{ $country->id }}" tabindex="-1" aria-hidden="true"
                                ...
                                <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
                                    ...
                                    <form class="" method="POST" action="{{ route('dropdowns.options.update', $country) }}">
                                        ...
                                        @method('PUT')
                                        <div class="mb-4">
                                            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __db('name') }}</label>
                                            <input type="text" name="value" value="{{ $country->value }}" required
                                                class="w-full border border-gray-300 rounded p-2">
                                        </div>
                                        <div class="mb-4">
                                            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __db('sort_order') }}</label>
                                            <input type="number" name="sort_order" value="{{ $country->sort_order ?? 0 }}"
                                                class="w-full border border-gray-300 rounded p-2">
                                        </div>
                                        <div class="mb-4">
                                            <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __db('status') }}</label>
            ...
            <div id="add-country" tabindex="-1" aria-hidden="true"
                ...
                <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 relative">
                    ...
                    <form method="POST" action="{{ route('dropdowns.countries.store') }}">
                        ...
                        <div class="mb-4">
                            <label class="block mb-2 font-medium">{{ __db('name') }} *</label>
