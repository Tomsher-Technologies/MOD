        ...
        <div>
            @can('add_dropdown_options')
                <a href="{{ route('countries.index') }}"
                    class="btn me-4 text-md mb-[-10px] !bg-[#009448] text-white rounded-lg ">
                    <span class="mr-2">{{ __db('countries') }}</span>
                </a>

                <a href="{{ route('dropdowns.bulk.import') }}"
                    class="btn me-8 text-md mb-[-10px] !bg-[#009448] text-white rounded-lg ">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" fill="none" stroke="white" stroke-width="2"
                    ...
                    <tbody>
                        @forelse($dropdowns as $key => $dropdown)
                            <tr class="odd:bg-[#F9F7ED]  align-[middle]">
                                ...
                                <td class="px-4 py-3 text-black">{{ $dropdown->name }}</td>
                                <td class="px-4 py-3 text-black text-center flex">
                                    <div class="relative inline-block">
                                        <svg class="copy-icon w-6 h-6 text-gray-800 dark:text-white cursor-pointer"
                                    ...
                                    <div class=" text-center gap-5">
                                        @can('edit_role')
                                            <a href="{{ route('dropdowns.options.show', $dropdown) }}"
                                                class="btn me-8 text-md  !bg-[#B68A35] text-white rounded-lg">
