            ...
            @foreach ($allModules as $i => $module)
                            ...
                            @foreach ($users as $k => $row)
                                <div class="user-card border border-gray-200 rounded-lg p-4 flex flex-col items-center text-center hover:shadow-lg transition-shadow relative"
                                    ...
                                    <div class="absolute top-2 left-2">
                                        <label for="switch-{{ $k }}{{ $i }}"
                                            class="relative inline-block w-9 h-5">
                                            <input type="checkbox" id="switch-{{ $k }}{{ $i }}"
                                                onchange="update_status(this)" value="{{ $row->id }}"
                                                class="sr-only peer" {{ $row->status == 1 ? 'checked' : '' }} />

                                            <div
                                                class="block bg-gray-300 peer-checked:bg-[#009448] w-9 h-5 rounded-full transition">
                                            </div>
                                            <div
                                                class="dot absolute left-1 top-1 bg-white w-3 h-3 rounded-full transition peer-checked:translate-x-5">
                                            </div>
                                        </label>
                                    </div>
                                    @if ($event->status === 0)
                                    ...
                                    <h4 class="text-lg font-semibold text-gray-900 text-center truncate w-full">
                                        {{ $row->user->name ?? '-' }}</h4>
                                    <p class="text-sm text-gray-600 mt-1 truncate w-full text-center">
                                        {{ $row->user->email ?? '-' }}</p>
                                    <span
                ...
                <div class="assignUserModal fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50"
                    ...
                    <div class="bg-white rounded-lg shadow-lg max-w-lg w-full p-6 relative">
                        ...
                        <h2 class="text-xl font-semibold mb-4">{{ __db('assign_users_to') }} {{ ucfirst(__db($module)) }}
                            {{ __db('module') }}</h2>
                        ...
                        <form method="POST" action="{{ route('events.assignUsers', $event->id) }}" class="assignUsersForm"
                            ...
                            <label class="block text-sm mb-1 text-gray-600 font-medium"
                                for="user_ids_{{ $module }}">{{ __db('select_users') }}</label>
                            ...
                            <label class="block text-sm mb-1 text-gray-600 font-medium"
                                for="role_id_{{ $module }}">{{ __db('select_role') }}</label>
                            ...
                            <div class="text-right">
                                <button type="submit"
                                    class="bg-[#a57d30] text-white px-4 py-2 rounded hover:bg-[#a57d30]">
                                    {{ __db('assign') }}
                                </button>
                            </div>
                        </form>
...
@section('script')
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const unassignUrlTemplate =
                "{{ route('events.unassignUser', ['event' => '__EVENT__', 'assigned' => '__USER__']) }}";
            ...
            document.querySelectorAll('.unassign-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const routeUrl = unassignUrlTemplate.replace('__EVENT__', eventId).replace(
                        '__USER__', assignedId);
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                success: function(data) {
                                    toastr.success(
                                        "{{ __db('user_unassigned') }}");
                                    $(`.user-card[data-id="${assignedId}"]`)
                                        .remove();
                                    setTimeout(function() {
            ...
            document.querySelectorAll('.openAssignModalBtn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    const modal = document.querySelector(
                        `.assignUserModal[data-module="${module}"]`);
                    if (modal) {
                        ...
                        if (result.isConfirmed) {
                            ...
                            fetch(form.action, {
                                ...
                                .then(data => {
                                    ...
                                    toastr.success(
                                        '{{ __db('user_assigned_success') }}');
                                    // Close modal
                                    form.closest('.assignUserModal').classList.add(
                                        'hidden');
                                    setTimeout(function() {
                                        window.location.reload();
                                    }, 2000);
