...
@section('content')
    <div class="">
        <div class="flex flex-wrap items-center justify-between gap-2 mb-6 mb-10">
            ...
            <a href="{{ route('staffs.index') }}" id="add-attachment-btn"
        ...
        <form action="{{ route('staffs.store') }}" method="POST" autocomplete="off">
            ...
            <div class="bg-white rounded-lg p-6 mb-10 mt-4">
                        ...
                        <div class="delegate-row border bg-white p-6 rounded bg-gray-100 mb-2">
                            <div class="grid grid-cols-12 gap-5">
                                ...
                                <div class="col-span-3">
                                    <label class="form-label">{{ __db('username') }} <span
                                ...
                                <div class="col-span-3">
                                    <label class="form-label">{{ __db('name') }} <span
                                            class="text-red-600">*</span></label>
                                ...
                                <div class="col-span-3">
                                    <label class="form-label">{{ __db('module') }} <span
                                            class="text-red-600">*</span></label>
                                    <select name="module" id="module"
                                        ...
                                        <option value="" {{ old('module') == '' ? 'selected' : '' }}>
                                            {{ __db('choose_option') }}</option>
                                        <option value="admin" {{ old('module') == 'admin' ? 'selected' : '' }}>
                                            {{ __db('admin') }}</option>
                                        <option value="delegate" {{ old('module') == 'delegate' ? 'selected' : '' }}>
                                            {{ __db('delegate') }}</option>
                                        <option value="escort" {{ old('module') == 'escort' ? 'selected' : '' }}>
                                            {{ __db('escort') }}</option>
                                        <option value="driver" {{ old('module') == 'driver' ? 'selected' : '' }}>
                                            {{ __db('driver') }}</option>
                                        <option value="hotel" {{ old('module') == 'hotel' ? 'selected' : '' }}>
                                            {{ __db('hotel') }}</option>
                                        <option value="top-management"
                                            {{ old('module') == 'top-management' ? 'selected' : '' }}>
                                            {{ __db('top_management') }}</option>
                                    </select>
                                    @error('module')
                                        <div class="text-red-600">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-span-3">
                                    <label class="form-label">{{ __db('role') }} <span
                                            class="text-red-600">*</span></label>
                ...
                <div class="flex justify-start items-center gap-5">
                    <button type="submit"
                        class="btn text-md  !bg-[#B68A35] text-white rounded-lg h-12 mr-4">{{ __db('submit') }}</button>

                    <a href="{{ route('staffs.index') }}"
                        class="btn text-md  !bg-[#637a85] border !border-[#637a85] !text-[#fff] rounded-lg h-12 mr-1">{{ __db('cancel') }}</a>
                </div>
            </div>
