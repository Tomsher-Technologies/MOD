    ...
    <div class="xl:col-span-12 h-full">
        <div class="bg-white h-full vh-100 max-h-full min-h-full rounded-lg border-0 p-6">
            <div class="flex items-center justify-between gap-12 mb-4">
                <form class="w-[100%]" action="{{ route('staffs.index') }}" method="GET">
                    <div class="relative">
                        ...
                        <div class="flex">
                            ...
                            <select name="module" id="module" class="block w-[20%] mr-2 p-2.5 !ps-10 text-secondary-light text-sm !border-[#d1d5db] rounded-lg">
                                <option value="" {{ (request()->module == "") ? 'selected' : '' }}>{{ __db('select_module') }}</option>
                                <option value="admin" {{ (request()->module == "admin") ? 'selected' : '' }}>{{ __db('admin') }}</option>
                                <option value="delegate" {{ (request()->module == "delegate") ? 'selected' : '' }}>{{ __db('delegate') }}</option>
                                <option value="escort" {{ (request()->module == "escort") ? 'selected' : '' }}>{{ __db('escort') }}</option>
                                <option value="driver" {{ (request()->module == "driver") ? 'selected' : '' }}>{{ __db('driver') }}</option>
                                <option value="hotel" {{ (request()->module == "hotel") ? 'selected' : '' }}>{{ __db('hotel') }}</option>
                            </select>
                ...
                <tbody>
                    @forelse ($users as $key => $staff)
                        <tr class="border-t">
                            ...
                            <td class="px-4 py-3 text-center text-black" dir="ltr">{{ $staff->military_number }}</td>
                            ...
                            <td class="px-4 py-3 text-center " dir="ltr">
                                @can('edit_staff')
                                    <a href="{{ route('staffs.edit', ['id' => base64_encode($staff->id)]) }}" title="{{ __db('edit') }}" class="w-8 h-8 bg-[#FBF3D6] text-primary-600 dark:text-primary-400 rounded-full inline-flex items-center justify-center">
...
<script>
    ...
    function update_status(el) {
        ...
        Swal.fire({
            ...
            confirmButtonColor: '#3085d6', // Adjust colors if needed
