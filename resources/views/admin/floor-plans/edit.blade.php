            ...
            <div class="grid grid-cols-12 gap-5">
                ...
                <div class="col-span-12">
                    ...
                    <div class="border rounded-lg p-4">
                        @php
                            $existing = old('existing_file_paths', $floorPlan->file_paths ?? []);
                        @endphp
                        @if (count($existing) > 0)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="files-container">
                                @foreach ($existing as $index => $filePath)
                                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded file-item"
                                        data-index="{{ $index }}">
                                        ...
                                        <a href="{{ asset('storage/' . $filePath) }}" target="_blank"
                                            class="text-[#B68A35] hover:underline mr-3">
                                            {{ __db('view') }}
                                        </a>
                                        ...
                                        <input type="hidden" name="existing_file_paths[]" value="{{ $filePath }}"
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                const hiddenInput = container.querySelector(
                                    `.existing-file-input[data-index="${index}"]`);
                                if (hiddenInput) {
                                    hiddenInput.remove();
                                }
                            }
