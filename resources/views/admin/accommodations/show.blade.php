...
<div class="dashboard-main-body">
    ...
    <div class="bg-white p-4 mt-4">
        <form id="filterForm" method="GET" action="{{ route('accommodations.show', base64_encode($hotel->id)) }}" class="mb-4">
            <div class="grid grid-cols-12 gap-5 room-row">
                ...
                <div class="col-span-3">
                    ...
                    <select name="room_type" id="room_type" class="select2 p-3 rounded-lg w-full border border-neutral-300 text-sm text-neutral-600 focus:border-primary-600 focus:ring-0" data-placeholder="{{ __db('select') }}">
                        <option value="">{{ __db('select') }}</option>
                        @foreach ($rooms as $roomType)
                            <option value="{{ $roomType->id }}" {{ request('room_type') == $roomType->id ? 'selected' : '' }}>{{ $roomType->roomType?->value }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 mt-3 h-full">
        <div class="xl:col-span-12 h-full">
            <div class="bg-white h-full vh-100 max-h-full min-h-full rounded-lg border-0 p-6">
                @php
                    $columns = [
                        ...
                        [
                            ...
                            'key' => 'military_number',
                            'render' => function ($escort) {
                                $searchUrl = route('escorts.index', ['search' => $escort->assignable?->military_number]);
                                return '<a href="' .
                                    $searchUrl .
                                    '" class="text-[#B68A35] hover:underline">' .
                        ...
                        [
                            ...
                            'key' => 'phone_number',
                            'render' => fn($escort) => '<span dir="ltr">' . e($escort->assignable?->phone_number) . '</span>',
                        ],
                        [
                            'label' => __db('gender'),
                            'key' => 'gender',
                        ...
                        [
                            ...
                            'key' => 'known_languages',
                            'render' => function ($escort) {
                                $ids = $escort->assignable?->spoken_languages ? explode(',', $escort->assignable?->spoken_languages) : [];
                ...
                <x-reusable-table :data="$escorts" :columns="$columns" :no-data-message="__db('no_data_found')" />
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 mt-3 h-full">
        <div class="xl:col-span-12 h-full">
            <div class="bg-white h-full vh-100 max-h-full min-h-full rounded-lg border-0 p-6">
                @php
                    $columns = [
                        ...
                        [
                            ...
                            'key' => 'military_number',
                            'render' => function ($driver) {
                                $searchUrl = route('drivers.index', ['search' => $driver->assignable?->military_number]);
                                return '<a href="' .
                                    $searchUrl .
                                    '" class="text-[#B68A35] hover:underline">' .
                        ...
                        [
                            'label' => __db('phone_number'),
                            'key' => 'phone_number',
                            'render' => fn($driver) => '<span dir="ltr">' . e($driver->assignable?->phone_number) . '</span>',
                        ],
                        [
                ...
                <x-reusable-table :data="$drivers" :columns="$columns" :no-data-message="__db('no_data_found')" />
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-between">
        <h2 class="font-semibold mb-0 !text-[22px]">{{ __db('external_accommodations') }} ({{ $externalMembers->count() }})</h2>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 mt-3 h-full">
        <div class="xl:col-span-12 h-full">
            <div class="bg-white h-full vh-100 max-h-full min-h-full rounded-lg border-0 p-6">
                    ...
                    $columns = [
                        ...
                        [
                            ...
                            'render' => function ($external) {
                                ...
                                $accommodation = $external->roomType?->roomType?->value . ' - ' . $external?->room_number ;
                                //;
                                return $accommodation ?? '-';
                            },
                        ],
                ...
                <x-reusable-table :data="$externalMembers" :columns="$columns" :no-data-message="__db('no_data_found')" />
            </div>
        </div>
    </div>
</div>
