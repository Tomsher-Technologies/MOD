                        ...
                        @forelse($interview_members as $key => $member)
                            <tr class="odd:bg-[#F9F7ED] align-[middle]">
                                ...
                                <td class="text-center px-4 py-3 text-black">
                                    {{ $member->event?->code ?? '-' }}
                                    @if ($member->event?->is_default)
                                        <span
                                            class="inline-block rounded bg-green-500 px-2 py-1 text-xs font-semibold text-white">
                                            Default
                                        </span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-black">{{ $member->name_en }}</td>
                                <td class="px-4 py-3 text-black text-center">{{ $member->name_ar }}</td>
                                <td class="px-4 py-3 text-center">
                                    <div class="gap-5">
                                        @if (strtolower($member->status) === 'active' || strtolower($member->status) === 'completed' || $member->status == 1)
                                            <span
                                                class="inline-block rounded px-2 py-1 text-xs font-semibold text-white bg-green-500">
                                                {{ __db('active') }}
                                            </span>
                                        @else
                                            <span
                                                class="inline-block rounded px-2 py-1 text-xs font-semibold text-black bg-yellow-400">
                                                {{ __db('inactive') }}
                                            </span>
                                        @endif
                                    </div>
