...
@section('content')
    ...
    <div class="space-y-4" x-data="delegateComponent()">
        ...
        <div id="delegate-container">
            <template x-for="(delegate, index) in delegates" :key="delegate.tmp_id">
                <div class="delegate-row border rounded p-4 grid grid-cols-12 gap-4 relative">
                    <button type="button" class="delete-row absolute top-2 end-2 text-red-600 hover:text-red-800"
                    ...
                    <div class="col-span-3">
                        ...
                        <select :name="`delegates[${index}][parent_id]`" x-model="delegate.parent_id"
                            ...
                            <template x-for="(parentDelegate, parentIndex) in delegates" :key="parentDelegate.tmp_id">
                    ...
                    <span class="col-span-12 border-t border-neutral-200 pt-6 mt-6 flex gap-8">
                        <div class="flex items-center gap-3">
                            <input type="checkbox" :id="`team-head-${index}`" :name="`delegates[${index}][team_head]`"
                                value="1" class="h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
    ...
    <script>
        function delegateComponent() {
            return {
                ...
                addDelegate() {
                    const newTmpId = Date.now() + Math.floor(Math.random() * 1000);
                    this.delegates.push({
                        tmp_id: newTmpId,
                        title_id: '',
                        name_ar: '',
