            ...
            <div class="flex flex-wrap gap-4">
                ...
                <div class="flex-1 min-w-[150px]">
                    <select name="import_type" class="select2 w-full border border-gray-300 rounded p-2">
                        ...
                        <option value="drivers" {{ request('import_type') == 'drivers' ? 'selected' : '' }}>{{ __db('drivers') }}</option>
                        <option value="escorts" {{ request('import_type') == 'escorts' ? 'selected' : '' }}>{{ __db('escorts') }}</option>
                        <option value="delegations" {{ request('import_type') == 'delegations' ? 'selected' : '' }}>{{ __db('delegations') }}</option>
                        <option value="delegates" {{ request('import_type') == 'delegates' ? 'selected' : '' }}>{{ __db('delegates') }}</option>
                        <option value="hotels" {{ request('import_type') == 'hotels' ? 'selected' : '' }}>{{ __db('hotels') }}</option>
                    </select>
                </div>
                ...
                $columns = [
                    [
                        ...
                        'render' => function ($log) {
                            ...
                            return '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ' . $typeClass . '">' . __db($log->import_type) . '</span>';
                        },
                    ],
                    [
                    ...
                    [
                        ...
                        'render' => function ($log) {
                            if ($log->status === 'success') {
                                return '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">' . __db('success') . '</span>';
                            } else {
                                return '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">' . __db('failed') . '</span>';
                            }
                        },
                    ],
                    [
                        ...
                        'render' => function ($log) {
                            if ($log->status === 'success') {
                                ...
                                return '<button onclick="showErrorDetails(' . $log->id . ')" class="text-red-600 hover:text-red-900">' . Str::limit($log->error_message, 50) . '</button>';
                            }
                        },
                    ],
                    [
                    ...
                    [
                        'label' => __db('actions'),
                        'render' => function ($log) {
                            return '<button onclick="showRowData(' . $log->id . ')" class="text-indigo-600 hover:text-indigo-900">' . __db('view_data') . '</button>';
                        },
                    ],
                ];
            @endphp
...
<script>
    function showErrorDetails(logId) {
        // In a real implementation, you would fetch the full error details via AJAX
        // For now, we'll just show the error message from the row
    ...
    function showRowData(logId) {
        // In a real implementation, you would fetch the row data via AJAX
        // For now, we'll just show the row data from the row
        const log = @json($logs->keyBy('id'));
    ...
    document.addEventListener('click', function(event) {
        ...
        if (rowDataModal && !rowDataModal.contains(event.target) && event.target.closest('#rowDataModal') === null) {
            closeRowDataModal();
        }
    });
</script>
