@extends('layouts.admin_account', ['title' => __db('all_escorts')])

@section('content')
    <div class="flex flex-wrap items-center justify-between gap-2 mb-6">
        <h2 class="font-semibold mb-0 !text-[22px]">{{ __db('all_escorts') }}</h2>
        <a href="{{ route('escorts.create') }}"
            class="bg-[#B68A35] text-white font-medium rounded-lg text-sm px-4 py-2">{{ __db('add_escort') }}</a>
    </div>
                    ...
                    <tbody>
                        @forelse ($escorts as $key => $escort)
                            <tr class="text-sm align-[middle]">
                                <td class="px-4 py-2 border border-gray-200" dir="ltr">
                                    {{ $key + 1 + ($escorts->currentPage() - 1) * $escorts->perPage() }}
                                </td>
                                <td class="px-4 py-3 border border-gray-200" dir="ltr">
                                    {{ $escort->name_en }}</td>
                                <td class="px-4 py-3 border border-gray-200" dir="ltr">
                                    {{ $escort->name_ar }}</td>
                                <td class="px-4 py-3 border border-gray-200" dir="ltr">
                                    {{ $escort->delegation->code ?? 'N/A' }}</td>
                                <td class="px-4 py-3 border border-gray-200" dir="ltr">
                                ...
                                <td class="px-4 py-3 border border-gray-200" dir="ltr">
                                    @if ($escort->status)
                                        <span
                                            class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">{{ __db('active') }}</span>
                                    @else
                                        <span
                                            class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">{{ __db('inactive') }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-start " dir="ltr">
                                    <a href="{{ route('escorts.show', $escort->id) }}" title="{{ __db('show') }}"
                        ...
                        @empty
                            <tr class="border-t">
                                <td class="px-4 py-3 text-center " colspan="8" dir="ltr">
                                    {{ __db('no__data_found') }}
                                </td>
                            </tr>
                        @endforelse
