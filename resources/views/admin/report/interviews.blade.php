             ...
             <form class="w-[75%] me-4"  method="GET"> 
                <div class="flex relative">
                    ...
                    <div class="flex flex-row w-[70%] gap-4">
                        ...
                        <div class="w-[50%]">
                            <select name="interviewee" data-placeholder="{{ __db('select') }} {{ __db('interviewee') }}" class="select2 rounded-lg border border-gray-300 text-sm w-full">
                                <option value="">{{ __db('select') }} {{ __db('interviewee') }}</option>

                                @foreach($delegates as $delegate)
                                    <option value="{{ $delegate['id'] }}" {{ request('interviewee') == $delegate['id'] ? 'selected' : '' }}>{{ $delegate['name'] }}</option>
                                @endforeach

                                @foreach($otherMembers as $other)
                                    <option value="{{ $other['id'] }}" {{ request('interviewee') == $other['id'] ? 'selected' : '' }}>{{ $other['name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
