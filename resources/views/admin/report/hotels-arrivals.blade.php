    ...
    <div>
        <div class="flex flex-wrap items-center justify-between gap-2 mb-6">
            ...
            <div class="flex gap-3 ms-auto">
                @directCanany(['export_arrival_hotels'])
                    <form action="{{ route('hotels-arrivals.bulk-exportPdf') }}" method="POST" style="display:inline;">
                        @csrf
                            ...
                            <select name="country[]" multiple data-placeholder="{{ __db('select') }} {{ __db('country') }}" class="select2 rounded-lg border border-gray-300 text-sm w-full" >
                                ...
                                @foreach (getAllCountries() as $option)
                                    <option value="{{ $option->id }}"
                                        {{ is_array(request('country')) && in_array($option->id, request('country')) ? 'selected' : '' }}>
                                        {{ $option->name }}
                                    </option>
                                @endforeach
                            </select>
