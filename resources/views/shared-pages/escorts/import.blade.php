<div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-2 mb-6">
        <h2 class="font-semibold mb-0 !text-[22px]">{{ __db('import') . ' ' . __db('escorts') }}</h2>
        <a href="{{ Session::has('import_escorts_last_url') ? Session::get('import_escorts_last_url') :  route('escorts.index') }}"
    ...
    <div class="bg-white h-full w-full rounded-lg border-0 p-30 mb-10">
        <div class="grid grid-cols-2 gap-8">
            <div class="border rounded-lg text-center p-10 block">
                <a href="{{ asset('assets/excel/sample_escorts.xlsx') }}" download
                    class="btn text-md mb-[-10px] !bg-[#B68A35] text-white rounded-lg w-full">
                    <svg class="w-6 h-6 text-white pe-2 inline" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                        width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 15v2a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-2m-8 1V4m0 12-4-4m4 4 4-4" />
                    </svg>
                    <span>{{ __db('download_sample_escorts_excel') }}</span>
                </a>

                <p class="text-sm mt-4">
                    {!! __db('download_sample_escorts_excel_description') !!}
                </p>
            </div>
            ...
            <div class="border  rounded-lg text-center p-10 block">
                <form action="{{ route('escorts.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <button type="submit"
                        class="btn text-md mb-[-10px] border !border-[#B68A35] text-[#B68A35] rounded-lg ">
