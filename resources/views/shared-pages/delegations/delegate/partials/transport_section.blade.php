...
<div class="bg-gray-50 rounded-lg p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-5 w-full">
        <div data-transport-section="{{ $type }}" data-mode-fields="flight"
            class="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-5">
            <div>
        ...
        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                <label class="form-label block mb-1 text-sm">{{ __db($type . '_status') }}:</label>
                <select name="{{ $type }}[status_id]"
                    class="p-3 rounded-lg w-full border border-neutral-300 text-sm">
                    <option value="">{{ __db('select_status') }}</option>
                    @foreach (getDropdown($type . '_status')->options as $status)
                        <option value="{{ $status->id }}" @if (old($type . '.status_id', $transport->status_id ?? '') == $status->id) selected @endif>
                            {{ $status->value }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <div data-transport-section="{{ $type }}" data-mode-fields="land sea" class="mt-4">
