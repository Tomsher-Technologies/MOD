  <div class="dashboard-main-body ">
    <div class="flex flex-wrap items-center justify-between gap-2 mb-6 mb-10">
        ...
        <a href="{{ getRouteForPage('escorts.index') }}" id="add-attachment-btn"
    ...
    <div class="bg-white h-full w-full rounded-lg border-0 p-6">
        <form action="{{ getRouteForPage('escorts.assign', $escort->id) }}" method="POST">
            @csrf
                ...
                <div class="grid  grid-cols-12 gap-2 items-end">
                        ...
                        <div class="grid grid-cols-2 gap-5 items-end">
                            <div>
                                ...
                                <select id="country_id" class="p-3 rounded-lg w-full border text-sm">
                                    ...
                                    @foreach ($countries->options as $country)
                    ...
                    <div class="col-span-12">
                        <div class="grid grid-cols-4 gap-5 items-end">
                            <div>
                                <label class="form-label block mb-1 text-gray-700 font-medium">Language:</label>
                                <select id="language_id" class="p-3 rounded-lg w-full border text-sm">
                                    <option selected="" disabled="">Select Language</option>
                                    {{-- @foreach ($languages->options as $language)
                    ...
                    <tbody>
                        ...
                        <tr class="text-sm align-middle">
                            <td class="px-4 py-2 border border-gray-200">
                                <input type="radio" name="delegation_id" value="{{ $delegation->id }}" class="w-4 h-4 !accent-[#B68A35]" {{ $escort->delegation_id == $delegation->id ? 'checked' : '' }}/>
                            </td>
                            <td class="px-4 py-2 border border-gray-200">{{ $delegation->code }}</td>
                            <td class="px-4 py-2 border border-gray-200">{{ $delegation->continent?->name_en }}</td>
                            <td class="px-4 py-2 border border-gray-200">{{ $delegation->country?->name_en }}</td>
                            <td class="px-4 py-2 border border-gray-200">{{ $delegation->team_head?->name_en }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchBtn.addEventListener('click', function() {
            ...
            tableRows.forEach(row => {
                ...
                if (languageId) {
                    const languages = Array.from(row.querySelectorAll('td:nth-child(6) span')).map(span => span.textContent.toLowerCase());
