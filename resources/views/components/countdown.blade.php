    @php
        ...
        $hasEventStarted = $startDate ? $now->greaterThanOrEqualTo(\Carbon\Carbon::parse($startDate)) : false;
        $hasEventEnded = $endDate ? $now->greaterThan(\Carbon\Carbon::parse($endDate)) : false;
            ...
            <div
                ...
                <div id="event-countdown" class="flex items-center justify-center ">
                    ...
                    <div class="countdown-item text-center flex flex-col items-center justify-center mx-2">
                        ...
                        <div class="countdown-label text-[0.6rem] mt-1 text-white uppercase">{{ __db('seconds') }}</div>
                    </div>
    ...
    @if ($hasEventStarted && !$hasEventEnded)
        @php
            ...
            $dayNumber = $startDate->startOfDay()->diffInDays(now()->startOfDay()) + 1;

            $totalDays = $startDate->startOfDay()->diffInDays($endDate->startOfDay()) + 1;
        @endphp
        <div class="flex items-center">
            <div
                class="countdown-timer bg-gradient-to-r from-[#ffcc00] via-[#ff9900] to-[#ffcc00] text-white py-2 px-4 rounded-lg animate-gradient">
